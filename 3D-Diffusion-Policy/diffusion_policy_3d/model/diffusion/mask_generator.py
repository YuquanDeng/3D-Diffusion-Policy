...
class PointcloudMaskGenerator(ModuleAttrMixin):
    def __init__(self,
        point_cloud_dim,
        # obs mask setup
        max_n_obs_steps=2, 
    ...
    def forward(self, shape, seed=None):
        ...
        steps = torch.arange(0, T, device=device).reshape(1,T).expand(B,T)
        obs_mask = (steps.T < obs_steps).T.reshape(B,T,1,1).expand(B,T,N,D)
        obs_mask = obs_mask & is_obs_dim
        ...
        if self.action_visible:
            action_steps = torch.maximum(
                ...
                torch.tensor(0,
                    dtype=obs_steps.dtype, 
                    device=obs_steps.device))
            action_mask = (steps.T < action_steps).T.reshape(B,T,1,1).expand(B,T,N,D)
            action_mask = action_mask & is_action_dim
